/* Auto-generated by utensor cli */
#ifndef __MY_MODEL_INTERFACE_H
#define __MY_MODEL_INTERFACE_H
#include "uTensor.h"

using namespace uTensor;

class My_model : public ModelInterface<1, 1> 
{
 public:
  enum input_names : uint8_t { input_0 };
  enum output_names : uint8_t { output_0 };
  My_model();
 protected:
  virtual void compute();
 private:
  // Operators
  TflmSymQuantOps::DequantizeOperator<float, int8_t> op_DequantizeOperator_000;

  ReferenceOperators::ReshapeOperator<int8_t> op_ReshapeOperator_001;

  TflmSymQuantOps::FullyConnectedOperator<int8_t> op_FullyConnectedOperator_002;

  TflmSymQuantOps::QuantizeOperator<int8_t, float> op_QuantizeOperator_003;

  TflmSymQuantOps::FullyConnectedOperator<int8_t> op_FullyConnectedOperator_004;

  TflmSymQuantOps::DepthwiseSeparableConvOperator<int8_t> op_DepthwiseSeparableConvOperator_005;

  ReferenceOperators::MaxPoolOperator<int8_t> op_MaxPoolOperator_006;

  // memory allocators
  localCircularArenaAllocator<256, uint16_t> ram_allocator;
  localCircularArenaAllocator<256, uint16_t> metadata_allocator;
};

#endif // __MY_MODEL_INTERFACE_H